...
export function useMarketFeed() {
    ...
    useEffect(() => {
        ...
        // Subscribe to all instruments (watchlist + positions + activeSymbol)
        const allInstruments = new Map<string, { segment: string; securityId: string }>();

        watchlist.forEach(item => {
            if (item.securityId && item.segment) allInstruments.set(item.securityId, { segment: item.segment, securityId: item.securityId });
        });
        ...
        // Add Active Symbol from Terminal Store
        const { activeSymbol, watchlists } = useTradingStore.getState();
        if (activeSymbol) {
            // Find in static map
            const staticConfig = getSymbolConfig(activeSymbol);
            if (staticConfig) {
                allInstruments.set(staticConfig.id, { segment: staticConfig.segment, securityId: staticConfig.id });
            } else {
                // Find in any watchlist
                for (const group of watchlists) {
                    const item = group.items.find(i => i.symbol === activeSymbol);
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [watchlist.length, positions.length, isConnected, useTradingStore.getState().activeSymbol]);
